...
NUNCA usar area_total_m2 para calcular preco_m2_imovel.
{
  ...
  "alertas": ["string acionÃÂ¡vel em linguagem clara"],
...
export function validarECorrigirAnalise(analise) {
  ...
  // 1. ÃÂrea usada para cÃÂ¡lculo Ã¢ÂÂ corrigir se usou total/real em vez de privativa
  ...
  if (analise.alertas) {
    analise.alertas = analise.alertas.filter(alerta => {
      ...
      if (a.includes('risco jurÃÂ­dico alto') && (analise.score_juridico || 0) >= 70) return false
      return true
    })
  ...
  if ((ocupLower === 'desocupado' || tituloLower.includes('nunca habitado') ||
    ...
    avisos.push('AJUSTE: imÃÂ³vel nunca habitado/desocupado Ã¢ÂÂ score_ocupacao ajustado')
