...
for (const s of SCORES) {
  ...
  return <div>
    <Hdr title={<>{formatTitulo(p)}{p.codigo_axis&&<span style={{fontSize:"10.5px",fontWeight:700,padding:"2px 8px",borderRadius:4,background:"#002B8010",color:"#002B80",border:"1px solid #002B8020",fontFamily:"monospace",letterSpacing:"0.5px",marginLeft:10,verticalAlign:"middle"}}>{p.codigo_axis}</span>}
      {p.motor_ia_usado&&<span title={`Analisado por: ${p.motor_ia_usado}`} style={{fontSize:"9px",padding:"2px 6px",borderRadius:4,background:p.motor_ia_usado.includes('gemini')?"#EEF2FF":p.motor_ia_usado.includes('deepseek')?"#F0FDF4":"#FFF7ED",color:p.motor_ia_usado.includes('gemini')?"#4338CA":p.motor_ia_usado.includes('deepseek')?"#065F46":"#92400E",border:"1px solid currentColor",borderOpacity:.3,marginLeft:4,verticalAlign:"middle",cursor:"help"}}>⚡ {p.motor_ia_usado.replace('gemini-2.5-flash','Gemini').replace('deepseek-chat','DeepSeek').replace('gpt-4o-mini','GPT-4o').replace('claude','Claude').split('-')[0]}</span>}{p.praca&&!isMercadoDireto(p.fonte_url,p.tipo_transacao)&&<span style={{display:"inline-block",background:p.praca>=2?"#FEF3C7":"#ECFDF5",color:p.praca>=2?"#D97706":"#065F46",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:4,marginLeft:6,verticalAlign:"middle"}}>{p.praca}ª PRAÇA{p.valor_minimo&&p.valor_avaliacao?` · mín. ${Math.round(p.valor_minimo/p.valor_avaliacao*100)}%`:p.praca>=2?" · mín. 35%":""}</span>}{p.num_leilao&&!p.praca&&!isMercadoDireto(p.fonte_url,p.tipo_transacao)&&<span style={{display:"inline-block",background:"#F1F5F9",color:"#64748B",fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:4,marginLeft:6,verticalAlign:"middle"}}>#{p.num_leilao}</span>}
    ...
    <div style={{padding:isPhone?"16px":"20px 28px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:isPhone?"1fr":"1fr 1fr",gap:"14px",marginBottom:"14px"}}>
        <div style={card()}>
          ...
          {[["Avaliação",fmtC(p.valor_avaliacao),K.t2],[isMercadoDireto(p.fonte_url,p.tipo_transacao)?"Preço pedido":"1ª praça",fmtC(isMercadoDireto(p.fonte_url,p.tipo_transacao)?(p.preco_pedido||p.valor_minimo):p.valor_minimo),K.amb],...(!isMercadoDireto(p.fonte_url,p.tipo_transacao)&&p.valor_minimo_2?[["⭐ 2ª praça (oportunidade)",fmtC(p.valor_minimo_2),'#059669']]:[]),...(parseFloat(p.desconto_percentual)>0.5?[["Desconto sobre avaliação",`${p.desconto_percentual}%`,K.t2]]:[]),["Desconto sobre mercado",p.desconto_sobre_mercado_pct_calculado?`${parseFloat(p.desconto_sobre_mercado_pct_calculado)>0?'':'+'}${parseFloat(p.desconto_sobre_mercado_pct_calculado).toFixed(1)}%`:"—",parseFloat(p.desconto_sobre_mercado_pct_calculado||0)>0?K.grn:K.red],["Preço/m² imóvel",p.preco_m2_imovel?`R$ ${Math.round(p.preco_m2_imovel).toLocaleString('pt-BR')}/m²`:"—",K.teal],["Preço/m² mercado",p.preco_m2_mercado?`R$ ${Math.round(p.preco_m2_mercado).toLocaleString('pt-BR')}/m²`:"—",K.t2],...(p.fator_homogenizacao&&p.fator_homogenizacao<1?[["Fator homogeneização",`${(p.fator_homogenizacao*100).toFixed(0)}%`,"#92400E"]]:[])].map(([l,v,c])=>(
            <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${K.bd}`}}>
              <span style={{fontSize:"12px",color:K.t3}}>{l}</span><span style={{fontSize:"12.5px",fontWeight:"600",color:c}}>{v}</span>
            </div>
          ))}
